...
export class StudentsComponent implements OnInit, AfterViewInit {
  ...
  poruka: string='Prikaži obrisane';
  deleted= {
    'color':'gray',
  ...
  fetchStudents(filter: string = '', page: number = 1, pageSize: number = 5): void {
    this.studentGetService.handleAsync({
      ...
      next: (data) => {
        ...
        this.studenti=this.dataSource.data;
        this.dataSource.data = this.isShown ? this.studenti : this.studenti.filter(x=>!x.isDeleted);
      },
      error: (err) => {
  ...
  show() {
    ...
    if(this.isShown){
      this.dataSource.data=this.studenti;
      this.poruka='Sakrij obrisane';
    }
    else{
      this.dataSource.data=this.studenti.filter(x=>!x.isDeleted);
      this.poruka='Prikaži obrisane';
    }
  }
